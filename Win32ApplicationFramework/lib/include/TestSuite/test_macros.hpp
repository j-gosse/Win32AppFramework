...
namespace winxframe
{
	#define CHECK(condition) TestRegistry::CurrentCase()->Check((condition), #condition, __FILE__, __LINE__)

	#define MSG_CHECK(condition, message) TestRegistry::CurrentCase()->Check((condition), std::string(message), __FILE__, __LINE__)

	#define CHECK_EQUAL(testValue, expectedValue) TestRegistry::CurrentCase()->CheckEqual((testValue), (expectedValue), #testValue, #expectedValue, __FILE__, __LINE__)

	#define CHECK_WITHIN(testValue, expectedValue, minimum) TestRegistry::CurrentCase()->CheckWithin((testValue), (expectedValue), (minimum), #testValue, #expectedValue, #minimum, __FILE__, __LINE__)
